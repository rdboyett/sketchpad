...
(function (root, factory) {
	...
	Raphael.fn.dragonDelete = function(subject, $element, options, callback) {
		var
			ft, i, mapped, timeout,
			paper = this,
			bbox  = subject.getBBox(true);
			if (subject.type=='text') {
				var toString = bbox.toString;
				
				bbox = {
					x:subject.attrs.x,
					y:subject.attrs.y,
					height:subject.attrs.height,
					width:subject.attrs.width,
					toString:toString
				}
			}
		...
		if ( subject.dragonDelete ) {
			return subject.dragonDelete;
		}
		...
		if (subject.type==='text') {
			ft = subject.dragonDelete = {
				// Keep track of transformations
				attrs: {
					x: subject.attrs.x,
					y: subject.attrs.y-8,
					size: { x: subject.attrs.width, y: subject.attrs.height },
					center: { x: (subject.attrs.x) + subject.attrs.width / 2, y: (subject.attrs.y-8) + subject.attrs.height / 2 },
					rotate: 0,
					scale: { x: 1, y: 1 },
					translate: { x: 0, y: 0 },
					ratio: 1,
		...
		}  //need to setup unplug when pointer is clicked off.
		...
		ft.updateHandles = function() {
			console.log('in ft.updateHandles');
			var
			...
			if ( ft.handles.center ) {
				ft.handles.center.disc.toFront().attr({
					x: ft.attrs.x + ft.attrs.translate.x - ft.attrs.padding,
					y: ft.attrs.y + ft.attrs.translate.y - ft.attrs.padding
				});
			}
			
			if ( ft.handles.xButton ) {
				ft.handles.xButton.back.toFront().attr({
					x: ft.attrs.x + ft.attrs.size.x + ft.attrs.translate.x,
					y: ft.attrs.y + ft.attrs.size.y + ft.attrs.translate.y
				});
				
				ft.handles.xButton.left.toFront().transform('T'+ ft.attrs.translate.x+','+ ft.attrs.translate.y);
				
				ft.handles.xButton.right.toFront().transform('T'+ ft.attrs.translate.x+','+ ft.attrs.translate.y);
			}
		...
		ft.showHandles = function() {
			...
			ft.hideHandles();  //hide any handles that might have been created........
			...
			if ( ft.opts.drag.indexOf('center') !== -1 ) {
				ft.handles.center = {};
				padding = ft.attrs.padding;
				
				ft.handles.center.disc = paper
					.rect(ft.attrs.x-padding, ft.attrs.y-padding, ft.attrs.size.x+(padding*2), ft.attrs.size.y+(padding*2), 4)
					.attr({
						'stroke-dasharray': '- ',
						fill:"rgba(0, 0, 0,0.1)"
					});
				ft.handles.center.disc.node.setAttribute('class', 'handle center');
				ft.handles.center.disc.node.setAttribute('cursor', 'move');
			}
			
			if ( ft.opts.drag.indexOf('xButton') !== -1 ) {
				ft.handles.xButton = {};
				padding = ft.attrs.padding;
				x = ft.attrs.x + ft.attrs.size.x + padding - 6;
				y = ft.attrs.y + ft.attrs.size.y + padding - 6;
				
				
				ft.handles.xButton.back = paper
					.rect(x, y, 15, 15, 2)
					.attr({
						fill:"blue"
					});
				ft.handles.xButton.left = paper
					.path("M"+x+','+y+'l8,8')
					.attr({
						'stroke-width':2,
						'stroke':'red'
					});
				ft.handles.xButton.right = paper
					.path("M"+(x+8)+','+(y)+'l-8,8')
					.attr({
						'stroke-width':2,
						'stroke':'red'
				...
				ft.handles.xButton.back.node.setAttribute('class', 'xButton');
				ft.handles.xButton.back.node.setAttribute('cursor', 'pointer');
				ft.handles.xButton.left.node.setAttribute('class', 'xButton');
				ft.handles.xButton.left.node.setAttribute('cursor', 'pointer');
				ft.handles.xButton.right.node.setAttribute('class', 'xButton');
				ft.handles.xButton.right.node.setAttribute('cursor', 'pointer');
			}
		...
		ft.hideHandles = function(opts) {
			...
			if ( ft.handles.xButton ) {
				ft.handles.xButton.back.remove();
				ft.handles.xButton.left.remove();
				ft.handles.xButton.right.remove();

				ft.handles.xButton = null;
			}
		...
		ft.setOpts = function(options, callback) {
			...
			if ( ft.opts.drag === true ) {
				ft.opts.drag = [ 'center', 'self', 'xButton'];
			}
		...
		ft.apply = function() {
			ft.items.map(function(item, i) {
				...
				if ( ft.opts.animate ) {
					...
					console.log('transformString animate');
					item.el.animate(
						{ transform: [
					...
					console.log('transformString apply');
					if (ft.items[i].el.type=='path') {
						item.el.transform([
							'R', rotate, center.x, center.y,
							'S', scale.x, scale.y, center.x, center.y,
							'T', translate.x, translate.y
						...
						$element.attr('transform','translate('+translate.x+','+translate.y+')');
					}
					
					//asyncCallback([ 'apply' ]);

					console.log(ft.items[i].el);
					console.log(subject);
					console.log(ft.items[i].el.type);
		...
		function isWithinBoundaries() {
			console.log('in isWithinBoundaries');
			return {
