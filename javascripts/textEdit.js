/*
 * Inline text editing tool for Rapha‘l 2.0 & compatible with Rapha‘l Free transform.
		...
		subject.inlineTextEditing = {
			...
			startEditing: function(){
				...
				var height        = originalBbox.height;
                                var x             = container.offsetLeft + subject.attrs.x + translateX - 5; //subtracted 5 to account for cursor position
				if (height<1) {
                                    var y         = (container.offsetTop + subject.attrs.y - height / 2 + translateY) - 27; //subtracted 25 to account for cursor position
                                }else{
                                    var y         = (container.offsetTop + subject.attrs.y - height / 2 + translateY)- 17;
                                }     
				var sTransform    = '';
				...
				// Re-apply stored transformation to the element and hide it
                                if (matrix==null) {
                                    subject.node.setAttribute("transform", "");
                                }else{
				...
				for(var z in oStyles){
					sStyles += z + ':' + oStyles[z] + ';';
                                ...
                                //add min-width
                                sStyles += 'min-width: 100px;min-height:50px;';
